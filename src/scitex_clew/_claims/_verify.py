...
"""Claim verification: verify_claim, verify_all_claims, _classify_claim."""
...
from ._db_ops import _ensure_claims_table, _resolve_claim, _update_claim_status, list_claims
from ._types import ClaimVerification, VerificationResult
...
def verify_claim(
    ...
    hash_cache : dict or None, optional
        Per-pass cache (resolved-path -> hash) threaded from
        ``verify_all_claims``. When provided, each file is hashed at most
        once within the pass. Pass ``None`` (default) to disable caching
...
def _classify_claim(result: Dict, *, strict: bool) -> int:
    ...
    * No ``source_file`` linked -> ``UNVERIFIED`` (the fabrication case: a
      claim registered against nothing computable).
    * Source file gone -> ``SOURCE_MISSING``.
    * Stored hash != current hash -> ``HASH_MISMATCH``.
    * Source hash matches but the claim was never marked verified -> ``UNVERIFIED``.
    * Source verified:
        * non-strict -> ``OK`` (hash match is the bar).
        * strict and chain NOT verified -> ``NO_LINEAGE``.
    ...
    # Source linked but neither verified nor explicitly failed.
    if not verified_at:
        return UNVERIFIED
...
def verify_all_claims(
    ...
    *,
    ...
    A single per-pass hash cache is shared across all ``verify_claim`` calls
    so a source file that is referenced by multiple claims is hashed from
    disk only once per ``verify_all_claims`` invocation.
    ...
    # Share a single hash cache across all claim verifications in this pass
    # so a source file referenced by multiple claims is hashed only once.
