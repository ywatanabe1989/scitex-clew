#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Python API introspection commands for scitex-clew."""
...
def _format_python_signature(func, multiline: bool = True, indent: str = "  ") -> tuple:
    """Format Python function signature with colored output.
...
def list_python_apis(verbose: int, max_depth: int, root_only: bool, as_json: bool):
    """List Python APIs (scitex-clew public API tree).
    ...
    Examples:
      ...
      clew list-python-apis -vv
      clew list-python-apis -vvv
      clew list-python-apis --json
    """
    import scitex_clew
    ...
    if as_json:
        ...
        click.secho(f"API tree of scitex-clew ({len(df)} items):", fg="cyan")
        legend = " ".join(
