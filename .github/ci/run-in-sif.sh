...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-clew's —
...
# so only scitex-clew's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_clew-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-clew's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-clew + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-clew's historical bare-uv/pip workflow so a
...
# scitex-clew's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-clew style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-clew-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-clew's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_clew --cov-report=xml --cov-report=term \
    -p no:cacheprovider
